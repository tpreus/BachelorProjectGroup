...
%as described in latex
    ...
    %new conditions
    %-------------
    %Bedingung dass Punke die in Y benachbart sind auch in X in anliegenden
    %Dreiecken liegen 
    ...
    if constrains(1) ==2 || constrains(1) ==3
        A_leq3_2 = sparse(2*my*fx ,fx*ny); %constrains for directeley neighboring triangles
    ...
    %condition that in every batch of triangles must be at least one vertex
    if constrains(3) == 1
        ...
        A_leq5 = [sparse(fx, 3*fx*ny), A_leq5 , sparse(fx, 2*mx*my)];%, sparse(fx , 2*my*fx);];
            ...
            for i = 1:2*mx
                ...
                for k = 1:fx %finde die genauen Dreiecke, in die v und w gelegt werden soll und die von {c1, c2} getrennt werden
                    if FX(k,1) == c1 && (FX(k,2) == c2 || FX(k,3) == c2) || (FX(k,2) == c1 && (FX(k,1) == c2 || FX(k,3) == c2)) || (FX(k,3) == c1 && (FX(k,1) == c2 || FX(k,2) == c2))
                        ...
                        A_leq4((e-1)*2*mx +i, (3*fx+ny) + (v-1)*fx + k) = e_dist; %P_p
